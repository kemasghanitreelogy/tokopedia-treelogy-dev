 ...
 *
 * Two of them, and neither is anything an invoice payload can carry.
 *
 * Where delivery charged to the buyer is credited is a company mapping, not an invoice
 * field - the sales invoice payload has no account on it anywhere. It is read here and
 * reported, never relied upon: the account it names is recorded in sources.js as
 * SHIPPING_ACCOUNT_NUMBER together with the whole investigation into why it is 7-70099
 * Other Income and not the 5030 the business first asked for. Reading it back is still
 * worth the request, because it is the one part of the booking policy that lives outside
 * this code and can be changed by a person without anybody noticing.
 *
...
export async function setUpChartOfAccounts({ dryRun = true, deadlineAt = null } = {}) {
  ...
  // What the books do with postage today, read rather than assumed. Nothing acts on the
  // answer - it is reported so that a mapping somebody changes in Jurnal shows up in the
  // next setup run instead of being discovered in a month-end variance.
